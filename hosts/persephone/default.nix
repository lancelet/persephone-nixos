...

{
  ...
  # NVIDIA VRAM fix for niri: NVIDIA doesn't properly release VRAM back to the pool
  # on Wayland compositors. Setting GLVidHeapReuseRatio=0 prevents VRAM exhaustion.
  ...
  environment.systemPackages = with pkgs; [
    (
      let
        blender-cuda = blender.override { cudaSupport = true; };
      in
      runCommand "blender" { nativeBuildInputs = [ makeWrapper ]; } ''
